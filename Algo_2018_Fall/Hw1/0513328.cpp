...
bool Find(vector<vector<int> > &young, int v) {
    int i(young.size() - 1), j(0);

    while (i >= 0 && j < young[0].size()) {
        cout << i << " " << j << endl;
        if (young[i][j] == v) return true;
...
void OutputResult(vector<vector<int> > &young) {
    fstream output_file;
    output_file.open("output2.txt", ios::out);
    for (int i = 0; i < young.size(); i++) {
        for (int j = 0; j < young[0].size(); j++) {
            output_file << young[i][j] << " ";
        }
        output_file << "\n";
    }
}
...
int main() {
    ...
    for (int i = 0; i < 9; i++) cout << ExtractMin(v) << endl;
    for (int i = 0; i < 9; i++) Insert(v, i);
    OutputResult(v);
    cout << Find(v, 8) << endl;
