...
def DecisionTree(train_data, feature_selected):
    ...
    rgr = tree.DecisionTreeRegressor(criterion="mse", max_depth=None, random_state=None)
...
for i in range(K):
    ...
    folded_data = review_data[0:0]

    for j in range(K):
        if(i!=j):
            folded_data = folded_data.append(divided_data[j])
    
    x_train = folded_data[all_feature]
    y_train = folded_data["Rating"]
    x_test = divided_data[i][all_feature]
    y_test = divided_data[i]["Rating"]
    ...
    for ind in range(lth):
        ...
        elif(vote_list[ind]<=3.5):
            vote_list_class[ind] = "1"
        elif(vote_list[ind]<=4):
            vote_list_class[ind] = "2"
        elif(vote_list[ind]<=4.5):
            vote_list_class[ind] = "3"
        elif(vote_list[ind]<=5):
    ...
    arr = confusion_matrix(vote_list_class, y_test_class, labels=['0', '1', '2', '3', '4'])
